...
async function handleCreateBranch(args) {
  ...
  try {
    ...
    try {
      ...
      const status = checkError.statusCode || checkError.response?.status;
      if (status !== 404) {
        throw checkError;
      }
    }
    ...
    if (args.from) {
      try {
        const baseBranchData = await githubRequest(`/repos/${owner}/${repo}/git/refs/heads/${args.from}`);
        baseSha = baseBranchData?.object?.sha;
      } catch (baseError) {
        const status = baseError.statusCode || baseError.response?.status;
        if (status !== 404) {
          throw baseError;
        }
      }
    ...
    if (!baseSha) {
      ...
      try {
        ...
        const status = defaultError.statusCode || defaultError.response?.status;
        if (status === 404) {
          try {
            const defaultBranchInfo = await githubRequest(`/repos/${owner}/${repo}/branches/${defaultBranch}`);
            baseSha = defaultBranchInfo?.commit?.sha;
          } catch (fallbackError) {
            const fallbackStatus = fallbackError.statusCode || fallbackError.response?.status;
      ...
      if (baseSha && args.from) {
        await auditLog('BRANCH_BASE_FALLBACK', {
          repo: `${owner}/${repo}`,
          requested: args.from,
          fallback: defaultBranch,
          sha: baseSha
        });
        resolvedFrom = defaultBranch;
      } else if (baseSha && !args.from) {
        resolvedFrom = defaultBranch;
      }
    }
    ...
    try {
      ...
      const status = createError.statusCode || createError.response?.status;
      const message = createError?.response?.data?.message || createError.message || '';
      if (status === 422 && /reference already exists/i.test(message)) {
        let existingSha;
        try {
          const existingRef = await githubRequest(`/repos/${owner}/${repo}/git/refs/heads/${branchName}`);
          existingSha = existingRef?.object?.sha;
        } catch (existingError) {
          throw createError;
        }
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ...
            from: resolvedFrom,
            sha: baseSha,
            message: `Successfully created branch '${branchName}' from '${resolvedFrom || 'default branch'}'`,
            url: `https://github.com/${owner}/${repo}/tree/${branchName}`
          })
        }
      ]
...
async function githubRequest(endpoint, params = {}, headers = {}, method = 'GET') {
  ...
  try {
    ...
    if (error && !error.statusCode && error?.response?.status) {
